...
select count(*) as dm_count from director_mapping;
select count(*) as genre_count from genre;
select count(*) as names_count from names;
select count(*) as rate_count from ratings;
select count(*) as role_count from role_mapping;
-- ------------------------------------------------------------------------------------------------------------------------------------------------
...
select count(*) as date_null from movie where date_published is NULL;
select count(*) as dura_null from movie where duration is NULL;
select count(*) as coun_null from movie where country is NULL;
select count(*) as gross_null from movie where worlwide_gross_income is NULL;
select count(*) as lang_null from movie where languages is NULL;
select count(*) as pro_com_null from movie where production_company is NULL;
...
count(*) as number_of_movies from movie group by month_num order by month_num;
...
+---------------+-------------------+ */
-- Type your code below:
select genre, round(avg(duration),2) as avg_duration from genre g left join movie m on g.movie_id=m.id
group by genre;
...
dense_RANK () OVER (ORDER BY count(movie_id) DESC) AS genre_rank from genre
group by genre)
...
with top_movies as
(
select title, round(avg(avg_rating),2) as avg_rating,
dense_rank() over (order by round(avg(avg_rating),2) desc) as movie_rank
from movie m left join ratings r on m.id=r.movie_id 
group by title
)select * from top_movies where movie_rank<=10;


select genre, title from movie inner join genre 
...
-- Type your code below:
select count(id) as movie_count from movie m inner join ratings r on m.id=r.movie_id
...
with votes_summary as
( select
count(case when lower(m.languages) like "%german%" then id end) as german_movie_count,
sum(case when lower(m.languages) like "%german%" then total_votes end) as german_movie_votes,
count(case when lower(m.languages) like "%italian%" then id end) as italian_movie_count,
sum(case when lower(m.languages) like "%italian%" then total_votes end) as italian_movie_votes
from movie m inner join ratings r 
on m.id=r.movie_id
) select round(german_movie_votes / german_movie_count, 2) as german_votes_per_movie,
round(italian_movie_votes / italian_movie_count, 2) AS italian_votes_per_movie from votes_summary;
...
SELECT COUNT(*) AS dob_nulls FROM names WHERE date_of_birth IS NULL;
SELECT COUNT(*) AS movies_nulls FROM names WHERE known_for_movies IS NULL;
...
count(case when date_of_birth is null then id end) as dob_nulls,
count(case when known_for_movies is null then id end) as movies_nulls
from names;
...
where g.genre in (select distinct genre from top_genre where genre_rank <= 3) and r.avg_rating > 8
...
from ratings r left join movie m on r.movie_id = m.id 
left join genre g on g.movie_id = m.id 
where g.genre = "Thriller" and total_votes > 25000;    
...
from top_prod 
) select * from ranked_prod where prod_comp_rank <=2;
